...
export default function Filters({ onFilterChange }) {
  ...
  return (
    <div className="w-full lg:w-4/4 bg-green-300 shadow-md rounded-lg p-5">
      <h2 className="text-xl font-bold mb-4">Filters</h2>
      ...
      <div className="mb-4">
        ...
        {categories.map((category) => (
          <button
            key={category}
            className={`block w-full text-left p-3 mb-2 rounded-lg text-sm ${selectedCategory === category ? "bg-blue-500 text-white" : "hover:bg-gray-200"}`}
            onClick={() => { setSelectedCategory(category); handleFilterChange(); }}
          >
            {category}
          </button>
      ...
      <div className="mb-4">
        ...
        {priceRanges.map((range) => (
          <button
            key={range.label}
            className={`block w-full text-left p-3 mb-2 rounded-lg text-sm ${selectedPriceRange.label === range.label ? "bg-blue-500 text-white" : "hover:bg-gray-200"}`}
            onClick={() => { setSelectedPriceRange(range); handleFilterChange(); }}
          >
            {range.label}
          </button>
      ...
      <div className="mb-4">
        ...
        {sizes.map((size) => (
          <button
            key={size}
            className={`block w-full text-left p-3 mb-2 rounded-lg text-sm ${selectedSize === size ? "bg-blue-500 text-white" : "hover:bg-gray-200"}`}
            onClick={() => { setSelectedSize(size); handleFilterChange(); }}
          >
            {size}
          </button>
      ...
      <div className="mb-4">
        ...
        {brands.map((brand) => (
          <button
            key={brand}
            className={`block w-full text-left p-3 mb-2 rounded-lg text-sm ${selectedBrand === brand ? "bg-blue-500 text-white" : "hover:bg-gray-200"}`}
            onClick={() => { setSelectedBrand(brand); handleFilterChange(); }}
          >
            {brand}
          </button>
